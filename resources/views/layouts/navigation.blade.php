<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    @php
        ...
        // Définition de la route du Dashboard selon le rôle
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('consultation.index')" :active="request()->routeIs('consultation.*')">
                     ...
                     <x-responsive-nav-link :href="route('dashboard.emploi')" :active="request()->routeIs('consultation.index')">
                {{ __('Floriane') }}
            </x-responsive-nav-link>

                    {{-- Ces boutons ne s'affichent que pour l'ADMIN --}}
                    @if($role === 'admin')
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ $user->name }} <span class="text-xs text-indigo-600">({{ ucfirst($role) }})</span></div>
